...
export class LoginPage implements OnInit {
  ...
  email: string;
  password: string;
  connectionErrorMessage = 'Le serveur distant n\'est pas joinable, veuillez vérifier votre connectivité';
  inviteToken = '';
  inviteKey = '';
  invalidInvite = false;
  inviteKeyError = '';
  ...
  ngOnInit() {
    ...
    const showNativeAppSuggestion = (this.platform.is('ios') && environment.showNativeAppSuggestionIOS) ||
    (this.platform.is('android') && environment.showNativeAppSuggestionAndroid);

    this.mobileLandScreen = this.platform.is('mobile') && environment.platform !== 'native' && showNativeAppSuggestion;
  ...
  async init() {
    ...
    this.subscriptions.push(this.authService.observeInviteToken().subscribe(inviteToken => {
  ...
  translatorSubmit(translator) {
    ...
    this.subscriptions.push(this.translatorServ.acceptRequest(this.inviteToken, translator).subscribe(res => {
      ...
      if (this.translatorAcceptError && this.translatorAcceptError.name === 'ERROR_INVITE_ACCEPTED' ||
        this.translatorAcceptError && this.translatorAcceptError.name === 'ERROR_NO_INVITE') {
        localStorage.removeItem('inviteToken');
        this.invite = null;
        this.inviteKey = null;
        this.inviteToken = null
  ...
  ngOnDestroy() {
    if (this.subInviteToken) { this.subInviteToken.unsubscribe(); }
...
END:VTIMEZONE
BEGIN:VEVENT
DTSTAMP:${this.datePipe.transform(new Date(), 'yyyyMMdd\THHmmss')}
UID:${Math.random().toString(36).substr(2, 9)}
DTSTART;TZID=Europe/Berlin:${this.datePipe.transform(date, 'yyyyMMdd\THHmmss')}
